...
def insert_log(time,temperature, pH, NTU, ORP):
    latest = select_newlogs(1)
    if time == latest[0][1]:
        return
    conn=connect_BigPeng()
    cur=conn.cursor()
...
def insert_elog(time, model, temperature, pH, NTU, ORP):
    latest = select_newelogs(1)
    if time == latest[0][1]:
        return
    conn=connect_BigPeng()
    cur=conn.cursor()
    try:
        cur.execute(f"""
        insert into e_logs
        (`time`,`model`,`temperature`,`pH`,`NTU`,`ORP`)
        values
        ('{time}', '{model}', '{temperature}', '{pH}', '{NTU}',' {ORP}');
        """)
        conn.commit()
